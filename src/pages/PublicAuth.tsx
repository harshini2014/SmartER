...
const PublicAuth = () => {
  ...
  const { isLoggedIn, loading, error, login, signup } = useAuth("public");
  ...
  useEffect(() => {
    if (isLoggedIn && !loading) navigate("/emergency", { replace: true });
  }, [isLoggedIn, loading]);
        ...
        <div className="px-5 py-8 max-w-sm mx-auto space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            ...
            <h2 className="font-display text-2xl font-bold text-foreground">
              {isSignUp ? "Public Sign Up" : "Public Login"}
            </h2>
            <p className="text-muted-foreground mt-1 text-sm">
              {isSignUp ? "Create your account for emergency help" : "Sign in to access emergency services"}
            </p>
          </motion.div>
            ...
            {isSignUp && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={phone} onChange={(e) => setPhone(e.target.value)} type="tel" placeholder="Enter your phone number" className="rounded-full h-12" required />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">Emergency Contact</label>
                  <Input value={emergencyContact} onChange={(e) => setEmergencyContact(e.target.value)} placeholder="Emergency contact name & number" className="rounded-full h-12" />
                </div>
              </>
            )}
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Email</label>
              <Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="you@example.com" className="rounded-full h-12" required />
            </div>
            <div className="space-y-2">
